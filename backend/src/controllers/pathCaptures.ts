...
export const startCapturePath = async (req: Request, res: Response) => {
  try {
    ...
    const activePath = await prisma.$queryRaw`
...
export const addPathPoint = async (req: Request, res: Response) => {
  try {
    ...
    const points: PathPoint[] = lastPoints.map((p: any) => ({
      ...point,
      lat: p.lat,
      lng: p.lng,
    ...
    const updatedPath = await prisma.$queryRaw`
...
export const completeCapturePath = async (req: Request, res: Response) => {
  try {
    ...
    const pathInfo = await prisma.$queryRaw`
      SELECT 
        cp.id,
        cp.territory_id,
    ...
    res.json({
      message: `Territory "${path.territory_name}" captured successfully by team "${userTeam.team.name}"!`,
      coverage: Math.round(coveragePercent),
      pathLength: Math.round(validation.path_length),
